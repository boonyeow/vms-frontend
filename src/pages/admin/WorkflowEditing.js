...
const WorkflowEditing = () => {
  ...
  const fetchFormList = async () => {
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(
            process.env.REACT_APP_ENDPOINT_URL + "/api/workflows/" + state.id,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          .then((res) => {
            ...
            res.data.forms.map((form) => {
              console.log(
                tempFormList.findIndex(
                  (item) =>
                    item.id.id == form.formId &&
                    item.id.revisionNo == form.revisionNo
                )
              );
              tempForms = [
                ...tempForms,
                {
                  pos: tempFormList.findIndex(
                    (item) =>
                      item.id.id == form.formId &&
                      item.id.revisionNo == form.revisionNo
                  ),
  ...
  return role != "ADMIN" || state.isFinal ? (
                      ...
                      <>
                        ...
                        <Grid item xs={11}>
                          <select
                            ...
                            {formList.map((input, k) => (
                              <option value={k}>
                                {input.name} (ID: {input.id.id}, Revision:{" "}
                                {input.id.revisionNo})
                              </option>
                            ))}
                          </select>
                        ...
                        <Grid item xs={0.5}>
                          {errors[index] && (
                            <Tooltip disableFocusListener title={errors[index]}>
